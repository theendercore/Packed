...
class PackScreenHandler(
    ...
    private val inventory: InvImpl = BackpackContentsComponent.EMPTY,
    ...
    private fun addPlayer(pInv: PlayerInventory) {
        ...
        for (i in 0..2) for (l in 0..8) {
            val idx = l + i * 9 + 9
            addSlot(PackPlayerSlot(pInv, idx, 8 + l * 18, 84 + i * 18))
        }
        // Hotbar
        for (i in 0..8) addSlot(PackPlayerSlot(pInv, i, 8 + i * 18, 142))
    }
    ...
    private fun inventory(): InvImpl = inventory // ?: error("Backpack has no inventory!")
